...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try { 
                ...
                reject(`Error creating a new block: ${err}`);
            }
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const diffMin = (currentTime - messageTime) / 60
            if (diffMin > 5) {
                reject('Message timeout (> 5 minutes)')
                return;
            }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (const block of self.chain) {
                ...
                if (!isValid) {
                    errorLog.push(`Block ${block.hash} / height ${block.height} is not valid`);
                }
                if (block.height > 0 && block.previousBlockHash !== self.chain[block.height - 1].hash) {
                    errorLog.push(`Block ${block.hash} / height ${block.height} has a different previousBlockHash (${block.previousBlockHash})`);
                }
            }
            resolve({
