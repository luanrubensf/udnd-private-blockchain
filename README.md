...
In the files can be found a file named `Blockchain.postman_collection.json`, that contains the requests to interact with this project.

## Generate an address

If you just use the `getnewaddress` command and try to sign a message, you will receive an error: `Address does not refer to key`.

To solve this, just generate an address with the `legacy` parameter:

```shell
getnewaddress "my_label" "legacy"
```

This address can be used to sign the message.
